...
%目标寄存器3个qubit，估计寄存器3个qubit
    ...
    %受控G门
    G = Grover3(A);
    W2 = kron(eye(2^5), CnU(2, 4, G));
    W3 = kron(eye(2^4), CnU([3,4], 5, G^2));
    W4 = kron(eye(2^3),CnU([5,6,7,8], 6, G^4));
    W5 = kron(eye(2^2),CnU([9:16], 7, G^8));
    W6 = kron(eye(2^1),CnU([17:32], 8, G^16));
    W7 = CnU([33:64], 9, G^32);
    psi = W7 * W6 * W5 * W4 * W3 * W2 * W1 * psi;

    %IQFT
    Q = IQFT(6);
    swap1 = CnX([17:32], 6) * XnC([2:2:32], 6) * CnX([17:32], 6);
    swap2 = CnX([5,6,7,8],4) * XnC([2,4,6,8],4) * CnX([5,6,7,8],4);
    swap3 = CnX([1],2) * XnC([1],2) * CnX([1],2);
    swap = swap1* kron3(I,swap2,I) * kron3(eye(4), swap3, eye(4));
...
%     %计算成功率
%     inner = 2^6/pi * asin(sqrt(1/2-1/2*res(j)^2)) + 1
%     if floor(inner) ~=1
